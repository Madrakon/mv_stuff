...
RegisterCommand("takehostage",function()
	if Config.EnableTH then
	callTakeHostage()
	elseif not Config.EnableTH then
	end
end)
...
function callTakeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification(_U('nonarmored'))
	end
	...
	if not takeHostage.InProgress and canTakeHostage then			
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				drawNativeNotification(_U('noonenearby'))
			end
		else
			drawNativeNotification(_U('noonenearby'))
		end
	...
	while true do 
		if takeHostage.type == "agressor" then
			...
			DisablePlayerFiring(PlayerPedId(),true)
	  		drawNativeText("<font face = 'RobotoCondensed'>[~r~G~w~] Pro Puštění hráče, [~y~H~w~] Pro zabití")
